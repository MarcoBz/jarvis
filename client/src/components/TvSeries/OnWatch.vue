...
export default {
  ...
  methods: {
    ...
    async singleClickEpisode(tmdbID, seasonNumber, episodeNumber){
      ...
      try{
        ...
        for (let i = 0; i < season.episodes.length; i++){
          if (season.episodes[i].isWatched || season.episodes[i].isPaused) seasonIsStarted = true
          if (!season.episodes[i].isWatched) seasonIsCompleted = false 
        }
        let seasonResponse 
        try{
    ...
    async changeTvSeriesStatus(tmdbID){
      let response 
        try{
          ...
          response = await userServiceTvSeries.changeTvSeriesStatus(this.user, tmdbID, body)
          ...
          try{
            ...
            if (response.status === 200){
              for (let i = 0; i < response.data.content.seasons.length; i++){
                let episodesArray = []
                for (let j = 0; j < response.data.content.seasons[i].episodes.length; j++){
                  episodesArray.push({
                    name: response.data.content.seasons[i].episodes[j].name,
                    airDate: response.data.content.seasons[i].episodes[j].airDate,
                    seasonNumber: response.data.content.seasons[i].episodes[j].seasonNumber,
                    episodeNumber: response.data.content.seasons[i].episodes[j].episodeNumber,
                    isWatched: false,
                    isPaused: false,
                    isSaved: false,
                    isAired: this.compareDate(response.data.content.seasons[i].episodes[j].airDate)
                  })
                }
                this.tvSeries.find(c => c.tmdbID == tmdbID).seasons.push({
                  airDate: response.data.content.seasons[i].airDate,
                  seasonNumber: response.data.content.seasons[i].seasonNumber,
                  numEpisodes: response.data.content.seasons[i].numEpisodes,
                  isOnWatch: false,
                  isCompleted: false,
                  isSaved: false,
                  episodes: episodesArray,
                  isAired: this.compareDate(response.data.content.seasons[i].airDate)
                })
              }
            }
              ...
              try{
                ...
                if (userResponse.status === 200){
                  for (let i = 0; i < userResponse.data.content.seasons.length; i++){
                    let seasonNumber = userResponse.data.content.seasons[i].seasonNumber
                    this.tvSeries.find(c => c.tmdbID == tmdbID).seasons.find(d => d.seasonNumber == seasonNumber).isSaved = true
                    if (userResponse.data.content.seasons[i].statusSeason == '1'){
                    ...
                    else if (userResponse.data.content.seasons[i].statusSeason == '-1'){
                    ...
                    else if (userResponse.data.content.seasons[i].statusSeason == '0'){
                      this.tvSeries.find(c => c.tmdbID == tmdbID).seasons.find(d => d.seasonNumber == seasonNumber).isCompleted = false
                      this.tvSeries.find(c => c.tmdbID == tmdbID).seasons.find(d => d.seasonNumber == seasonNumber).isOnWatch = false
                    }
                    for (let j = 0; j < userResponse.data.content.seasons[i].episodes.length; j++){
                        let episodeNumber = userResponse.data.content.seasons[i].episodes[j].episodeNumber
                        ...
                        if (parseInt(userResponse.data.content.seasons[i].episodes[j].statusEpisode) == '1'){
                        ...
                        else if (parseInt(userResponse.data.content.seasons[i].episodes[j].statusEpisode) == '-1'){
                        ...
                        else if (parseInt(userResponse.data.content.seasons[i].episodes[j].statusEpisode) == '0'){
                          this.tvSeries.find(c => c.tmdbID == tmdbID).seasons.find(d => d.seasonNumber == seasonNumber).episodes.find(f => f.episodeNumber == episodeNumber).isWatched = false
                          this.tvSeries.find(c => c.tmdbID == tmdbID).seasons.find(d => d.seasonNumber == seasonNumber).episodes.find(f => f.episodeNumber == episodeNumber).isPaused = false
                        }
                    }
    ...
    compareDate(airDate){
      ...
      let airDateYear = parseInt(airDate.split('-')[0])
      let airDateMonth = parseInt(airDate.split('-')[1])
      let airDateDay= parseInt(airDate.split('-')[2])
      let isAlreadyAired  
